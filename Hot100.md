...
滑动窗口，用`pcnt[26]`数组来存放字符串p中出现字母的次数，`scnt[26]`随着窗口的向后滑动不断更新，直接比较`pcnt`和`scnt`判断是否当前窗口与p是字母异位词(C++ 中的vector也可以直接用等号比较)
...
要找出和为 `K` 的子数组个数，可以用哈希表存储键为前缀和，值为出现的次数，则只需返回键 `pre - k` 对应的值

由于查询使用哈希表，不需要遍历，前缀和也不需要用数组存，只需要维护一个变量用来记录遍历到当前位置的前缀和

```python
def subarraySum(self, nums: List[int], k: int) -> int:
    pre = 0
    ans = 0
    mp = {0: 1} # 初始化，前缀和为0的子数组至少有一个
    ...
    for num in nums:
        ...
        # 使用get查找字典中的键值对，如果查找不到返回0
        # 直接查找，若查找不到会返回keyerror
...
![alt text](img/SlideWindowsMaxNumber.png)

### 算法思想

每次要记录滑动窗口中的最大值，可以使用**大顶堆**，每次更新大顶堆中的元素。

为了降低弹栈的时间复杂度，可以采用**懒删除**，在大顶堆里存放数值以及索引，删除堆顶中索引不再窗口中的元素。

`python`中只有小顶堆，所以添加元素时存放元素的负值。
